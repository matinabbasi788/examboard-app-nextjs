...
// Mock allocations data for testing
...
export async function getAllocations(examId?: number): Promise<Allocation[]> {
  ...
  // Filter mock data if exam ID provided
  let mockData = MOCK_ALLOCATIONS;
  if (examId) {
    mockData = MOCK_ALLOCATIONS.filter(a => a.exam === examId);
  }
  
  if (!token) {
    console.warn('No access token - using mock allocations data');
    return mockData;
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`Failed to fetch allocations (${res.status}) - using mock data`);
      return mockData;
    }
...
export async function getAllocationsForExams(examIds: number[]): Promise<Map<number, Allocation>> {
  ...
  try {
    ...
    if (!token) {
      console.warn('No access token - using mock allocations');
      MOCK_ALLOCATIONS.forEach(a => {
        allocationMap.set(a.exam, a);
      });
